...
admin.verify = function(id){
    ...
    ui.confirm('确定要进行此操作吗？', {
        yes: function () {
            $.post(U('classroom/AdminTeacher/doVerify'),{id:id,state:1},function(msg){
 ...
 * seo记录操作
 */
admin.doaction = function(id,action,type,state){
    id = ("undefined" == typeof(id) || id == '') ? admin.getChecked() : id;
    if(id == ''){
    ...
    ui.confirm('确定要进行此操作吗？', {
        yes: function () {
            $.post(U('admin/Seo/doaction'+action),{id:id,type:type,state:state},function(obj){
                if(obj.state == 1){
