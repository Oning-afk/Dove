...
<div id="container" class="so_main">
    ...
    <div class="Toolbar_inbox">
        ...
        <a onclick="admin.delCouponAll('delCoupon')" class="btn_a"><span>禁用</span></a>
        <a onclick="admin.addCoupon('OnlineCard','addCoupon')" class="btn_a"><span>添加</span></a>
    ...
    <div class="list" id='list'>
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
                ...
                <th class="line_l"   >优惠券编码</th>
                <th class="line_l"   >满</th>
                <th class="line_l"   >减</th>
                <th class="line_l"   >有效期（天）</th>
                <th class="line_l"   >优惠券数量</th>
            ...
            <#list cardVoucherPageBean.list as cardVoucher>
                <tr>
                    <td><input type="checkbox" value="${cardVoucher.id}" onclick="admin.checkon(this)"  name="checkbox"></td>
                    <td>${cardVoucher.id}</td>
                    <td>${cardVoucher.institutionsId}</td>
                    <td>${cardVoucher.cardVoucherNo}</td>
                    <td>${cardVoucher.institutionsId}</td>
                    <td>${cardVoucher.institutionsId}</td>
                    <td>${cardVoucher.validiteDay}</td>
                    <td>
                        已发送<span style="color: red">${cardVoucher.receivedNumber}</span>|
                        未发送<span style="color: red">${cardVoucher.cardVoucherNumber}</span>
                    </td>
                    <td>${cardVoucher.toReceivedDate ? string('yyyy-MM-dd hh:mm:ss')}</td>
                    <td>
            ...
            <tr>
                <td colspan="11" align="right">
                    ...
                    <#if cardVoucherPageBean.totalRecord &gt; 0>
                        <a href="/operating/findCardVoucher?page=1">首页</a>
                        ...
                        <#if cardVoucherPageBean ?? && cardVoucherPageBean.pageNum == 1 && cardVoucherPageBean.pageNum != cardVoucherPageBean.totalPage>
                            <#list cardVoucherPageBean.start..cardVoucherPageBean.end as i>
                                <#if cardVoucherPageBean.pageNum == i>
                                    ${i}
                                <#else >
                                    <a href="/operating/findCardVoucher?page=${i}">${i}</a>
                                </#if>
                            </#list>
                            <a href="/operating/findCardVoucher?page=${cardVoucherPageBean.pageNum + 1}">下一页</a>
                        </#if>
                        <!-- 如果当前不是第一页,也不是最后一页时,则有上一页跟下一页 -->
                        <#if cardVoucherPageBean.pageNum &gt;1 && cardVoucherPageBean.pageNum &lt; cardVoucherPageBean.totalPage>
                            <a href="/operating/findCardVoucher?page=${cardVoucherPageBean.pageNum-1}">上一页</a>
                            <#list cardVoucherPageBean.start..cardVoucherPageBean.end as i>
                                <#if cardVoucherPageBean.pageNum == i>
                                    ${i}
                                <#else >
                        ...
                        <a href="/operating/findCardVoucher?page=${cardVoucherPageBean.totalPage}">尾页</a>
                    </#if>
