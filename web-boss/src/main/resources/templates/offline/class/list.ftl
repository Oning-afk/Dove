...
<div id="container" class="so_main">
    ...
    <div class="tit_tab">
        <ul>
            <li><a href="/offline/class/findOfflineClassList?isDel=0&offlineClassIsAudit=1" class="on">线下课</a></li>
            <li><a href="/offline/class/toAdd" >添加</a></li>
        </ul>
    </div>
    <!-- END TAB框 -->
    ...
    <div class="form2" id='search_form' >
        <form>
            ...
            <dl class="lineD" id='dl_course_name'>
                ...
                <dd>
                    <#if bean?? && bean.offlineClassName ??>
                        <input name="offlineClassName" id="offlineClassName" type="text" value="${bean.offlineClassName}"  class='s-txt' style="width:200px" onblur="this.className='s-txt'" onfocus="this.className='s-txt-focus'" >
                    <#else>
                        <input name="offlineClassName" id="offlineClassName" type="text" value=""  class='s-txt' style="width:200px" onblur="this.className='s-txt'" onfocus="this.className='s-txt-focus'" >
                    </#if>
                </dd>
            </dl>
            <dl class="lineD" id='dl_teacher_name'>
                <dt>讲师名称：</dt>
                <dd>
                    <#if bean?? && bean.teacherName ??>
                        <input name="teacherName" id="teacherName" type="text" value="${bean.teacherName}"  class='s-txt' style="width:200px" onblur="this.className='s-txt'" onfocus="this.className='s-txt-focus'" >
                    <#else>
                        <input name="teacherName" id="teacherName" type="text" value=""  class='s-txt' style="width:200px" onblur="this.className='s-txt'" onfocus="this.className='s-txt-focus'" >
                    </#if>
                </dd>
            </dl>
            <div class="page_btm">
    ...
    <div class="list" id='list'>
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
            ...
            <#list list.list as offlineClass>
                <tr>
                    <td><input type="checkbox" value="${offlineClass.id}" onclick="admin.checkon(this)"  name="checkbox"></td>
                    <td>${offlineClass.id}</td>
                    <td>${offlineClass.offlineClassName}</td>
                    <td>${offlineClass.offlineClassPrice}</td>
                    <td>${offlineClass.offlineClassBuyNum}</td>
                    <td>${offlineClass.offlineClassTotalPeople}</td>
                    <td>
                        <#if offlineClass.offlineClassImage ??>
                            <img src="${offlineClass.offlineClassImage}" style="width: 50px;height: 50px;">
                            <#else >
                            <span>暂无封面</span>
                        </#if>
                    </td>
                    <td>${offlineClass.offlineClassCreater}</td>
                    <td>${offlineClass.offlineClassTeacher}</td>
                    <td>
                        <#if offlineClass.offlineClassIsFree==0>
                            <span style="color:#ff4d5c;">否</span>
                            <#else>
                            <span style="color:#45ff2d;">是</span>
                        </#if>
                    </td>
                    <td>${offlineClass.createTime ? string('yyyy-MM-dd hh:mm:ss')}</td>
                    <td>
                        <a onclick="admin.echoOffline(${offlineClass.id})" class="btn_a"><span>修改</span></a>
                    </td>
                </tr>
            </#list>
            <tr>
...
<script>
    function searchList(page) {
        var id = $("#id").val();
        var offlineClassName = $("#offlineClassName").val();
        var teacherName = $("#teacherName").val();
        admin.searchList(page,id,offlineClassName,teacherName,1,0);
    }
