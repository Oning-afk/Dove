...
<div id="container" class="so_main">
    ...
    <div class="form2" id='search_form' >
        <form action="http://try.51eduline.com/index.php?app=mall&mod=AdminGoodsAddress&act=index&dosearch=1" method="post">
            ...
            <dl class="lineD" id='dl_name'>
                ...
                <dd>
                    ...
                    <input name="name" id="form_name" type="text" value=""  class='s-txt' style="width:200px" onblur="this.className='s-txt'" onfocus="this.className='s-txt-focus'" >
    ...
    <div class="Toolbar_inbox">
        <!-- <div class="page right"><span>共40条</span> <a class='current'>1</a><a p='2' href='/index.php?app=mall&mod=AdminGoodsAddress&act=index&p=2'>2</a><a p='2' href='/index.php?app=mall&mod=AdminGoodsAddress&act=index&p=2' class='next'>下一页</a></div> -->
        <a onclick="admin.fold('search_form')" class="btn_a"><span>搜索</span></a><a onclick="admin.delAddressAll('delAddress')" class="btn_a"><span>删除</span></a><a onclick="admin.exportGoodAddress()" class="btn_a"><span>导出</span></a>
    </div>
    <!-- END TOOLBAR -->
    ...
    <div class="list" id='list'>
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
                ...
                <th class="line_l"   >用户UID</th>
                <th class="line_l"   >地区</th>
                <th class="line_l"   >地址</th>
                <th class="line_l"   >姓名</th>
                <th class="line_l"   >电话</th>
                <th class="line_l"   >是否启用</th>
                <th class="line_l"   >默认地址</th>
                <th class="line_l"   style="width:auto;" >操作</th>
            </tr>
            <#list receivingPageBean.list as receiving>
                <tr>
                    <td><input type="checkbox" value="${receiving.id}" onclick="admin.checkon(this)"  name="checkbox"></td>
                    <td>${receiving.id}</td>
                    <td>${receiving.userId}</td>
                    <td>${receiving.regionName}</td>
                    <td>${receiving.address}</td>
                    <td>${receiving.name}</td>
                    <td>${receiving.phone}</td>
                    <td>
                        <#if receiving.enable == 1>
                            <span style='color: green;'>正常</span>
                        </#if>
                        <#if receiving.enable == 0>
                            <span style='color: red;'>禁用</span>
                        </#if>
                    </td>
                    <td>
                    <#if receiving.defaultAddress == 0>是
                    </#if>
                    <#if receiving.defaultAddress == 1>否
                    </#if>
                    </td>
                    <td>
                        <a href="">禁用</a>|
            ...
            <tr>
                <td colspan="11" align="right">
                    <!-- 如果总页数为0 -->
                    <#if receivingPageBean.totalRecord == 0>
                    </#if>
                    <#if receivingPageBean.totalRecord &gt; 0>
                        <a href="/receiving/findReceiving?page=1">首页</a>
                        <!-- 如果当前为第一页时,就没有上一页这个超链接显示 -->
                        <#if receivingPageBean ?? && receivingPageBean.pageNum == 1 && receivingPageBean.pageNum != receivingPageBean.totalPage>
                            <#list receivingPageBean.start..receivingPageBean.end as i>
                                <#if receivingPageBean.pageNum == i>
                                    ${i}
                                <#else >
                                    <a href="/receiving/findReceiving?page=${i}">${i}</a>
                                </#if>
                            </#list>
                            <a href="/receiving/findReceiving?page=${receivingPageBean.pageNum + 1}">下一页</a>
                        </#if>
                        <!-- 如果当前不是第一页,也不是最后一页时,则有上一页跟下一页 -->
                        <#if receivingPageBean.pageNum &gt;1 && receivingPageBean.pageNum &lt; receivingPageBean.totalPage>
                            <a href="/receiving/findReceiving?page=${receivingPageBean.pageNum-1}">上一页</a>
                            <#list receivingPageBean.start..receivingPageBean.end as i>
                                <#if receivingPageBean.pageNum == i>
                                    ${i}
                                <#else >
                        ...
                        <!-- 如果当前为最后一页时,则没有下一页 -->
                        <#if receivingPageBean.pageNum == receivingPageBean.totalPage && receivingPageBean.pageNum != 1>
                            <a href="/receiving/findReceiving?page=${receivingPageBean.pageNum-1}">上一页</a>
                            <#list receivingPageBean.start..receivingPageBean.end as i>
                                <#if receivingPageBean.pageNum == i>
                                    ${i}
                        ...
                        <a href="/receiving/findReceiving?page=${receivingPageBean.totalPage}">尾页</a>
                    </#if>
                    共${receivingPageBean.totalRecord}条
                </td>
            </tr></table>
    </div>
    <!-- END LIST -->
